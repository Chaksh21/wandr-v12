...
window.WandrFeatures = {
  install: function(inst){
    ...
    inst.rvHome = function(c){
      ...
      const enterSetupWith = (k)=>{ if (!st.user.authed){ this.setState({ view:'auth', auScreen:'auth', auAfter:'newtrip', grSheetOpen:false, grInviteOpen:false, lvFoodOpen:false, plSheetOpen:false, exFilterOpen:false }); this.toast('Quick sign-in - then straight to your trip'); return; } this.setState({ view:'setup', suScreen:'s2', newDestKey:k, suDestConfirmed:true, suDestText:'', suLockedTheme:null, suConfirmed:false, suStaged:[], suPath:null, suTripName:'', suGroupName:'', suMembers: MEMBERS.filter(m=>m.id==='me').map(m=>Object.assign({}, m)), suExtraCount:0, suDateStart:null, suDateEnd:null, qzStep: st.user.name ? 1 : 0, dkIdx:0, suSeedPids:[] }); };
      ...
      return {
        ...
        openSearch,
        addTrip:()=>{ if (!st.user.authed){ this.setState({ view:'auth', auScreen:'auth', auAfter:'newtrip', grSheetOpen:false, grInviteOpen:false, lvFoodOpen:false, plSheetOpen:false, exFilterOpen:false }); this.toast('Quick sign-in - then straight to your trip'); return; } this.setState({ view:'setup', suScreen:'s2', suLockedTheme:null, suConfirmed:false, suStaged:[], suPath:null, suTripName:'', suGroupName:'', suMembers: MEMBERS.filter(m=>m.id==='me').map(m=>Object.assign({}, m)), suExtraCount:0, suDateStart:null, suDateEnd:null, newDestKey:'shimla', suDestConfirmed:false, suDestText:'', qzStep: st.user.name ? 1 : 0, dkIdx:0, suSeedPids:[] }); },
    ...
    inst.rvSetup = function(c){
      ...
      const parsedFields = [
        { label:'HOTEL', value:'A central stay in '+funnelDest.name, lowConfidence:false, onEdit:()=>this.toast('Search for your actual hotel - coming soon') },
      ].map(f=>Object.assign(f, { confDisplay:'none', rowBg:'transparent' }));
      const tasks = [
      ...
      const suOptions = optionsFor(funnelDest, suPrefs, ALL_COUNT).map(o=>{
        ...
        const preview = shownStops.map(s=>({ name:s.name, photo:photoUrl(s.photo), q:s.photo, moreDisplay:'none', moreLabel:'', nameDisplay:'block' }));
        if (overflow){ const last=o.allStops[3]; preview.push({ name:last.name, photo:photoUrl(last.photo), q:last.photo, moreDisplay:'flex', moreLabel:'+'+extraCount, nameDisplay:'none' }); }
      ...
      return {
        ...
        qzBack: ()=>{ if (this._qzT){ clearTimeout(this._qzT); this._qzT=null; } if (st.qzStep>0) this.setState({ qzStep:st.qzStep-1 }); else this.setState({ view:'home', suLockedTheme:null, suConfirmed:false, suStaged:[], suPath:null, suTripName:'', suGroupName:'', suMembers: MEMBERS.filter(m=>m.id==='me').map(m=>Object.assign({}, m)), suExtraCount:0, suDateStart:null, suDateEnd:null, suDestConfirmed:false, suDestText:'' }); },
        ...
        qzOptions: (st.qzStep===1
          ? [ ['slow','ill_quiz_pace_slow','Slow & soft','long breakfasts, no rush'], ['balanced','ill_quiz_pace_balanced','Balanced','full but easy'], ['packed','ill_quiz_pace_packed','Packed','see absolutely everything'] ].map(([k,img,label,sub])=>({ img:'./assets/illustrations/'+img+'.png', label, sub, ring: st.suPace===k?'2px solid var(--accent)':'1px solid var(--border)', bg: st.suPace===k?'var(--accent-weak)':'#fff', onPick:()=>{ this.setState({ suPace:k }); if (this._qzT) clearTimeout(this._qzT); this._qzT=setTimeout(()=>{ this._qzT=null; this.setState(x=>({ qzStep:Math.min(3,x.qzStep+1) })); }, 260); } }))
          : [ ['shoestring','ill_quiz_budget_shoestring','Save where I can','watch every rupee'], ['comfort','ill_quiz_budget_comfort','Balanced','sensible with treats'], ['splurge','ill_quiz_budget_splurge','Treat me','the trip IS the treat'] ].map(([k,img,label,sub])=>({ img:'./assets/illustrations/'+img+'.png', label, sub, ring: st.suBudget===k?'2px solid var(--accent)':'1px solid var(--border)', bg: st.suBudget===k?'var(--accent-weak)':'#fff', onPick:()=>{ this.setState({ suBudget:k }); if (this._qzT) clearTimeout(this._qzT); this._qzT=setTimeout(()=>{ this._qzT=null; this.setState(x=>({ user:Object.assign({},x.user,{ prefs:{ pace:x.suPace, styleTags:x.suStyleTags } }) })); this.suNav('s3'); }, 260); } }))),
        ...
        scr_s5:st.suScreen==='s5', scr_s6:st.suScreen==='s6',
        ...
        extraCount: st.suExtraCount, extraMinusOpacity: st.suExtraCount<=0?'0.4':'1',
        extraMinus:()=>this.setState(x=>({ suExtraCount: Math.max(0, x.suExtraCount-1) })),
        extraPlus:()=>this.setState(x=>({ suExtraCount: Math.min(20, x.suExtraCount+1) })),
        peopleTotalLabel: (st.suMembers.length+st.suExtraCount)+' going · '+st.suMembers.length+' on Wandr'+(st.suExtraCount? ' + '+st.suExtraCount+' more':''),
        peopleDone:()=>{ const named=st.suMembers.filter(m=>m.name.trim()); const guests=Array.from({length:st.suExtraCount},(_,i)=>({ id:'guest'+i, name:'Guest '+(i+1), bg:'#C7C1B8', status:'joined', type:'app', role:'COLLABORATOR', sub:'Added without a Wandr account' })); MEMBERS.length=0; MEMBERS.push(...named, ...guests); APP_MEMBERS = MEMBERS.filter(m=>m.type!=='companion'); ALL_COUNT = MEMBERS.length; APP_COUNT = APP_MEMBERS.length; this.suNav(st.suEditReturn||'s8'); },
        peopleBack:()=>this.suNav(st.suPeopleBack||'s8'),
        ...
        seeSample:()=>{ const tt=st.trips.find(x=>x.status==='active')||st.trips[0]; if (tt) openTripId(tt.id); },
        goS1:()=>this.suNav('s1'), exitSetupAsk:()=>this.setState({ suExitConfirm:true }), exitSetupCancel:()=>this.setState({ suExitConfirm:false }), showExitConfirm:!!st.suExitConfirm, exitSetup:()=>this.setState({ view:'home', suExitConfirm:false, suLockedTheme:null, suConfirmed:false, suStaged:[], suPath:null, suTripName:'', suGroupName:'', suMembers: MEMBERS.filter(m=>m.id==='me').map(m=>Object.assign({}, m)), suExtraCount:0, suDateStart:null, suDateEnd:null, suDestConfirmed:false, suDestText:'' }), goS2:()=>this.suNav('s2'), goS3:()=>this.suNav('s3'), goS4:()=>this.suNav('s4'),
        goS5:()=>{ if (st.suEditReturn){ const ret=st.suEditReturn; this.setState({ suEditReturn:null }); this.suNav(ret); } else { this.suNav('s5'); } }, goS6:()=>this.suNav('s6'), goS12:()=>this.suNav('s12'),
        ...
        goS4b2:()=>this.suNav('sPeople'),
        goToPeople:()=>this.setState({ suEditReturn:'s5', suPeopleBack:'s4b', suScreen:'sPeople' }),
        s4NextBg: st.suDestConfirmed?'var(--accent)':'var(--well)', s4NextFg: st.suDestConfirmed?'#fff':'var(--sec)', s4NextShadow: st.suDestConfirmed?'0 10px 26px rgba(255,90,31,.3)':'none',
        scr_s4b:st.suScreen==='s4b',
        dow:['S','M','T','W','T','F','S'], calCells, rangeLabel, dayCountLabel, dayCountDisplay,
        pickImport:()=>{ this.setState({ suPath:'import', suConfirmed:false, suImportOpen:true }); }, pickScratch:()=>{ this.setState({ suPath:'scratch', suScreen:'s6' }); },
        showImportPopup: !!st.suImportOpen,
        parsedFields, notConfirmed:!st.suConfirmed, isConfirmed:st.suConfirmed, confirmBooking:()=>{ this.setState({ suConfirmed:true, suImportOpen:false }); this.suNav('s6'); },
        closeImportPopup:()=>this.setState({ suImportOpen:false }),
        s6Cta: st.suStaged.length?'Synthesise plan':'Skip & draft',
        tasks,
        tripCode:t.code, destName: (V==='setup' ? funnelDest.name : dest.name), suMapPins, contributors, contributorsSummary, lockedLabel, invite:()=>{ const link='wandr.app/join/'+t.code; try{ navigator.clipboard.writeText(link); } catch(e){} this.toast('Link copied · '+t.code); },
        copyCode:()=>{ try{ navigator.clipboard.writeText(t.code); } catch(e){} this.toast('Copied · '+t.code); },
        suOptions, showSeasonBanner, seasonBannerText, lockCtaLabel: suPicked ? ('Lock '+lockedLabel) : 'Select a draft to lock', lockCtaBg: suPicked?'var(--accent)':'var(--well)', lockCtaFg: suPicked?'#fff':'var(--sec)',
    ...
    inst.rvTrips = function(c){
      ...
      return {
        ...
        tpNewTrip:()=>{ if (!st.user.authed){ this.setState({ view:'auth', auScreen:'auth', auAfter:'newtrip', grSheetOpen:false, grInviteOpen:false, lvFoodOpen:false, plSheetOpen:false, exFilterOpen:false }); this.toast('Quick sign-in - then straight to your trip'); return; } this.setState({ view:'setup', suScreen:'s2', suLockedTheme:null, suConfirmed:false, suStaged:[], suPath:null, suTripName:'', suGroupName:'', suMembers: MEMBERS.filter(m=>m.id==='me').map(m=>Object.assign({}, m)), suExtraCount:0, suDateStart:null, suDateEnd:null, newDestKey:'shimla', suDestConfirmed:false, suDestText:'', qzStep: st.user.name ? 1 : 0, dkIdx:0, suSeedPids:[] }); },
      };
    };
    ...
    inst.rvExplore = function(c){
      ...
      const enterSetupWithX = (k, theme)=>{ if (!st.user.authed){ this.setState({ view:'auth', auScreen:'auth', auAfter:'newtrip', grSheetOpen:false, grInviteOpen:false, lvFoodOpen:false, plSheetOpen:false, exFilterOpen:false }); this.toast('Quick sign-in - then straight to your trip'); return; } this.setState({ view:'setup', suScreen:'s2', newDestKey:k, suDestConfirmed:true, suDestText:'', suLockedTheme:theme||null, suConfirmed:false, suStaged:[], suPath:null, suTripName:'', suGroupName:'', suMembers: MEMBERS.filter(m=>m.id==='me').map(m=>Object.assign({}, m)), suExtraCount:0, suDateStart:null, suDateEnd:null, qzStep: st.user.name ? 1 : 0, dkIdx:0, suSeedPids:[] }); };
      ...
      const exSoon = ['Varanasi','Leh','Munnar','Pondicherry','Coorg','Darjeeling'].map(n=>({ name:n, onNotify:()=>{ this.toast("Draft a "+n+" trip - be the first"); if (!st.user.authed){ this.setState({ view:'auth', auScreen:'auth', auAfter:'newtrip', suDestText:n }); return; } this.setState({ view:'setup', suScreen:'s2', newDestKey:'shimla', suDestConfirmed:false, suDestText:n, suLockedTheme:null, suConfirmed:false, suStaged:[], suPath:null, suTripName:'', suGroupName:'', suMembers: MEMBERS.filter(m=>m.id==='me').map(m=>Object.assign({}, m)), suExtraCount:0, suDateStart:null, suDateEnd:null, qzStep: st.user.name ? 1 : 0, dkIdx:0, suSeedPids:[] }); } }));
      ...
      return {
        ...
        shStart: shTop ? (()=>{ if (!st.user.authed){ this.setState({ view:'auth', auScreen:'auth', auAfter:'newtrip' }); this.toast('Quick sign-in - then straight to your trip'); return; } const pids=shTop.rows.length?st.shortlist.filter(x=>x.destKey===shTop.key).map(x=>x.placeId):[]; this.setState({ view:'setup', suScreen:'s2', newDestKey:shTop.key, suDestConfirmed:true, suDestText:'', suSeedPids:pids, suLockedTheme:null, suConfirmed:false, suStaged:[], suPath:null, suTripName:'', suGroupName:'', suMembers: MEMBERS.filter(m=>m.id==='me').map(m=>Object.assign({}, m)), suExtraCount:0, suDateStart:null, suDateEnd:null, qzStep: st.user.name ? 1 : 0, dkIdx:0 }); }) : (()=>{}),
      };
    };
    ...
    inst.rvCuratedPreview = function(c){
      ...
      const cpBuild = ()=>{
        ...
        this.setState({ view:'setup', suScreen:'s2', newDestKey:st.cpDestKey, suDestConfirmed:true, suDestText:'', suLockedTheme:theme.id, suConfirmed:false, suStaged:[], suPath:null, suTripName:'', suGroupName:'', suMembers: MEMBERS.filter(m=>m.id==='me').map(m=>Object.assign({}, m)), suExtraCount:0, suDateStart:null, suDateEnd:null, qzStep: st.user.name ? 1 : 0, dkIdx:0, suSeedPids:[] });
      };
      return {
    ...
    inst.rvPlaceSearch = function(c){
      ...
      return {
        ...
        psdStartTrip: ()=>{ if (!detailDest) return; if (!st.user.authed){ this.setState({ view:'auth', auScreen:'auth', auAfter:'newtrip', newDestKey:st.psDestKey, suDestConfirmed:true, suLockedTheme:null }); this.toast('Quick sign-in - then straight to your plan'); return; } this.setState({ view:'setup', suScreen:'s2', newDestKey:st.psDestKey, suDestConfirmed:true, suDestText:'', suLockedTheme:null, suConfirmed:false, suStaged:[], suPath:null, suTripName:'', suGroupName:'', suMembers: MEMBERS.filter(m=>m.id==='me').map(m=>Object.assign({}, m)), suExtraCount:0, suDateStart:null, suDateEnd:null, qzStep: st.user.name ? 1 : 0, dkIdx:0, suSeedPids: detailPlace?[detailPlace.id]:[] }); },
      };
    };
    ...
    inst.rvAuth = function(c){
      ...
      return {
        ...
        auPersonas: [ ['planner','ill_signup_persona_planner','The planner','Likes to plan every detail'], ['floater','ill_signup_persona_floater','The floater','Goes along with the group'], ['spark','ill_signup_persona_spark','The spark','Suggests spontaneous ideas'] ].map(([k,img,label,sub])=>({ img:'./assets/illustrations/'+img+'.png', label, sub, onPick:()=>this.setState({ auScreen:'profile' }) })),
