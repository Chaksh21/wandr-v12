  ...
  install: function(inst){
    ...
    inst.rvShared = function(c){
      ...
      return {
        ...
        icStar: ICONS['star'], icStarFill: ICONS['star-fill'], icBookmark: ICONS['bookmark-simple'],
    ...
    inst.rvHome = function(c){
      ...
      const enterSetupWith = (k)=>{ if (!st.user.authed){ this.setState({ view:'auth', auScreen:'auth', auAfter:'newtrip', grSheetOpen:false, grInviteOpen:false, lvFoodOpen:false, plSheetOpen:false, exFilterOpen:false }); this.toast('Quick sign-in - then straight to your trip'); return; } this.setState({ view:'setup', suScreen:'s2', newDestKey:k, suDestConfirmed:true, suDestText:'', suLockedTheme:null, suConfirmed:false, suStaged:[], suPath:null, suTripName:'', suGroupName:'', suDateStart:null, suDateEnd:null, qzStep: st.user.name ? 1 : 0, dkIdx:0, suSeedPids:[] }); };
      ...
      return {
        ...
        openSearch,
        addTrip:()=>{ if (!st.user.authed){ this.setState({ view:'auth', auScreen:'auth', auAfter:'newtrip', grSheetOpen:false, grInviteOpen:false, lvFoodOpen:false, plSheetOpen:false, exFilterOpen:false }); this.toast('Quick sign-in - then straight to your trip'); return; } this.setState({ view:'setup', suScreen:'s2', suLockedTheme:null, suConfirmed:false, suStaged:[], suPath:null, suTripName:'', suGroupName:'', suDateStart:null, suDateEnd:null, newDestKey:'shimla', suDestConfirmed:false, suDestText:'', qzStep: st.user.name ? 1 : 0, dkIdx:0, suSeedPids:[] }); },
    ...
    inst.rvSetup = function(c){
      ...
      return {
        ...
        qzBack: ()=>{ if (this._qzT){ clearTimeout(this._qzT); this._qzT=null; } if (st.qzStep>0) this.setState({ qzStep:st.qzStep-1 }); else this.setState({ view:'home', suLockedTheme:null, suConfirmed:false, suStaged:[], suPath:null, suTripName:'', suGroupName:'', suDateStart:null, suDateEnd:null, suDestConfirmed:false, suDestText:'' }); },
    ...
    inst.rvTrips = function(c){
      ...
      return {
        ...
        tpNewTrip:()=>{ if (!st.user.authed){ this.setState({ view:'auth', auScreen:'auth', auAfter:'newtrip', grSheetOpen:false, grInviteOpen:false, lvFoodOpen:false, plSheetOpen:false, exFilterOpen:false }); this.toast('Quick sign-in - then straight to your trip'); return; } this.setState({ view:'setup', suScreen:'s2', suLockedTheme:null, suConfirmed:false, suStaged:[], suPath:null, suTripName:'', suGroupName:'', suDateStart:null, suDateEnd:null, newDestKey:'shimla', suDestConfirmed:false, suDestText:'', qzStep: st.user.name ? 1 : 0, dkIdx:0, suSeedPids:[] }); },
      };
    };
    ...
    inst.rvExplore = function(c){
      ...
      const enterSetupWithX = (k, theme)=>{ if (!st.user.authed){ this.setState({ view:'auth', auScreen:'auth', auAfter:'newtrip', grSheetOpen:false, grInviteOpen:false, lvFoodOpen:false, plSheetOpen:false, exFilterOpen:false }); this.toast('Quick sign-in - then straight to your trip'); return; } this.setState({ view:'setup', suScreen:'s2', newDestKey:k, suDestConfirmed:true, suDestText:'', suLockedTheme:theme||null, suConfirmed:false, suStaged:[], suPath:null, suTripName:'', suGroupName:'', suDateStart:null, suDateEnd:null, qzStep: st.user.name ? 1 : 0, dkIdx:0, suSeedPids:[] }); };
      ...
      const exSoon = ['Varanasi','Leh','Munnar','Pondicherry','Coorg','Darjeeling'].map(n=>({ name:n, onNotify:()=>{ this.toast("Draft a "+n+" trip - be the first"); if (!st.user.authed){ this.setState({ view:'auth', auScreen:'auth', auAfter:'newtrip', suDestText:n }); return; } this.setState({ view:'setup', suScreen:'s2', newDestKey:'shimla', suDestConfirmed:false, suDestText:n, suLockedTheme:null, suConfirmed:false, suStaged:[], suPath:null, suTripName:'', suGroupName:'', suDateStart:null, suDateEnd:null, qzStep: st.user.name ? 1 : 0, dkIdx:0, suSeedPids:[] }); } }));
      ...
      return {
        ...
        shStart: shTop ? (()=>{ if (!st.user.authed){ this.setState({ view:'auth', auScreen:'auth', auAfter:'newtrip' }); this.toast('Quick sign-in - then straight to your trip'); return; } const pids=shTop.rows.length?st.shortlist.filter(x=>x.destKey===shTop.key).map(x=>x.placeId):[]; this.setState({ view:'setup', suScreen:'s2', newDestKey:shTop.key, suDestConfirmed:true, suDestText:'', suSeedPids:pids, suLockedTheme:null, suConfirmed:false, suStaged:[], suPath:null, suTripName:'', suGroupName:'', suDateStart:null, suDateEnd:null, qzStep: st.user.name ? 1 : 0, dkIdx:0 }); }) : (()=>{}),
      };
    };
    ...
    inst.rvCuratedPreview = function(c){
      ...
      const cpBuild = ()=>{
        ...
        this.setState({ view:'setup', suScreen:'s2', newDestKey:st.cpDestKey, suDestConfirmed:true, suDestText:'', suLockedTheme:theme.id, suConfirmed:false, suStaged:[], suPath:null, suTripName:'', suGroupName:'', suDateStart:null, suDateEnd:null, qzStep: st.user.name ? 1 : 0, dkIdx:0, suSeedPids:[] });
      };
      return {
    ...
    inst.rvPlaceSearch = function(c){
      ...
      return {
        ...
        psdStartTrip: ()=>{ if (!detailDest) return; if (!st.user.authed){ this.setState({ view:'auth', auScreen:'auth', auAfter:'newtrip', newDestKey:st.psDestKey, suDestConfirmed:true, suLockedTheme:null }); this.toast('Quick sign-in - then straight to your plan'); return; } this.setState({ view:'setup', suScreen:'s2', newDestKey:st.psDestKey, suDestConfirmed:true, suDestText:'', suLockedTheme:null, suConfirmed:false, suStaged:[], suPath:null, suTripName:'', suGroupName:'', suDateStart:null, suDateEnd:null, qzStep: st.user.name ? 1 : 0, dkIdx:0, suSeedPids: detailPlace?[detailPlace.id]:[] }); },
      };
    };
