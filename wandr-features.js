  ...
  install: function(inst){
    ...
    inst.rvGroup = function(c){
      ...
      return {
        ...
        grCode:t.code, syncTag:'SYNCED', headerSub: grGroup ? (ALL_COUNT+' travellers · '+MEMBERS.filter(m=>m.status==='joined').length+' joined · '+MEMBERS.filter(m=>m.status==='pending').length+' pending') : 'Solo trip',
